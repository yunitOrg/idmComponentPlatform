<template>
    <div class="u-emoji">
        <a-popover :placement="placement" overlayClassName="emoji-popover-container" trigger="click" @visibleChange="visibleChange">
            <template #content>
                <div style="width: 251px">
                    ...
                    <div class="emoji-body select-none">
                        <div class="emjio-container" :style="{ transform: `translateX(${offsetX}%)` }">
                            <div v-for="(list, index) in emojis" :key="index" class="emoji-wrapper common-scrollbar_style">
                                <div style="padding: 0 5px">
                                    <span v-for="(value, key) in list" :key="key" class="emoji-item" @click="emitter('addEmoji', key as unknown as string)">
                                        <a-image :preview="false" :src="value" :title="String(key)" class="emoji" style="width: 24px; height: 24px; margin: 5px"></a-image>
                                    </span>
                                </div>
                            </div>
                        </div>
...
import { EmojiApi } from '@/undraw-ui/components'
...
const visibleChange = (isShow: boolean) => {
    if (isShow) bus.emit('comment-popover-show', 'popover-1')
}
</script>
