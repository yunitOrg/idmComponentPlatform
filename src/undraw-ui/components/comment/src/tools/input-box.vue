...
import { isEmpty, isNull, isImage, createObjectURL } from '@/undraw-ui/util'
import { InjectionEmojiApi, EditorInstance, UEmoji, UEditor, EmojiApi } from '@/undraw-ui/index'
...
const emit = defineEmits<{ (e: 'hide', event: Event): void; (e: 'close'): void }>()
...
const onSubmit = () => {
    submit({
        content: props.replay ? `回复 <span style="color: var(--u-color-success-dark-2);">@${props.replay}:</span> ${content.value}` : content.value,
...
bus.on('comment-popover-show', () => {
    nextTick(() => {
        popperRef.value = document.querySelector('div.ant-popover')
    })
})
...
const change = () => {
    ...
    if (files) {
        for (let i = 0; i < files.length; i++) {
            const fileName = files[i].name // 获取当-前文件的文件名
            const url = createObjectURL(files[i]) // 获取当前文件对象的URL
            files2.value.push(files[i])
            // 判断文件是否是图片类型
            if (isImage(fileName)) {
                imgList.value.push(url)
            } else {
                message.warn('请选择图片类型文件!')
            }
        }
    }
}
