...
import { ReplyApi } from '@/undraw-ui/index'
...
const data = computed(() => {
    ...
    if (props.data) {
        const length = props.data.list.length
        data = {
    ...
    if (!state.over) {
        const tmp = data.list.slice(0, replyShowSize)
        data.list = tmp
    }
    return data
})
