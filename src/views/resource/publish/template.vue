    ...
    <div class="cp-upload" style="width:960px">
            ...
            <ARow>
                <ACol :span="7">
                    <div class="title-left">
                        <div class="name">{{ !state.queryId ? '发布模板' : '编辑模板' }}</div>
                    </div>
                </ACol>
                <ACol :span="17">
                    ...
                    <ASpin :spinning="formData.formLoading">
                        ...
                        >
                            <a-alert v-if="state.noExistsComponentList.length>0" type="warning" show-icon closable :style="{ marginBottom: '20px' }">
                                <template #message>
                                    该模板包内包含了{{ state.componentList.length }}个组件，其中{{ state.noExistsComponentList.length }}个组件不存在 <a href="javascript:;" @click="openDetailModal">查看</a>，但不影响模板页面的在线预览（缺少的组件建议还是维护上云）
                                </template>
                            </a-alert>
                            <a-form-item name="id" hidden />
                            <a-form-item name="newGuid" hidden />
                            <a-form-item name="status" hidden />
                            <a-form-item name="businessId" hidden />
                            <a-form-item
                                name="title"
                                label="模板名称"
                                extra="请输入1-50个字符的模板中文名，以便更好的理解模板的作用"
                                :rules="[{ required: true, message: '必填!' }, { max: 50, min: 1, message: '请输入1-50个字符' }]"
                            >
                                <a-input v-model:value="formData.title" placeholder="请输入" />
                            </a-form-item>
                            <a-form-item name="businessId" label="模板ID">
                                <span class="ant-form-text">{{ formData.businessId }}</span>
                            </a-form-item>
                            ...
                            >
                                <a-radio-group v-model:value="formData.type" :options="state.templatetypeList" />
                            </a-form-item>
                            <a-form-item
                                name="contentType"
                                label="类型"
                                :rules="[{ required: true, message: '必填!' }]"
                            >
                                <a-radio-group v-model:value="formData.contentType" :options="state.contenttypeList" />
                            </a-form-item>
                            <a-form-item
                                name="ownerType"
                                label="所属"
                            ...
                            <a-form-item name="currentRemark" label="模板简介">
                                <a-textarea v-model:value="formData.currentRemark" placeholder="请输入" />
                            </a-form-item>
                            <a-form-item name="tags" label="模板标签">
                                <AsyncSelect
                            ...
                            >
                                <IFileUpload
                                    v-model:value="formData.currentZipPath"
                                    :paramsData="{upFileType: 'template'}"
                                />
                            </a-form-item>
                            <a-form-item name="markdownContent" extra="文档编辑后会存入表单，随表单一并保存" label="模板详情文档">
                                <MdEditorButton tip="文档编辑后会存入表单，随表单一并保存" v-model:value="formData.markdownContent" :path="formData.markdownPath" />
                            </a-form-item>
                            <a-form-item v-show="formData.updateTime || formData.createTime" name="updateTime" label="更新时间">
                                <span class="ant-form-text">{{ formData.updateTime || formData.createTime }}</span>
                            </a-form-item>
                            <a-form-item :wrapper-col="{ span: 24}" :style="{ padding: '20px 0','text-align': 'center' }">
                                ...
                                <a-button html-type="submit" :style="{ marginLeft: '20px' }" @click="() => formData.status = undefined">保存模板</a-button>
        ...
        >
            <a-alert type="warning" show-icon closable :style="{ marginBottom: '20px' }">
                <template #message>
                    该模板包内包含了{{ state.componentList.length }}个组件，其中{{ state.noExistsComponentList.length }}个组件不存在，但不影响模板页面的在线预览（缺少的组件建议还是维护上云），如果已经发布了不存在的组件，请再次点击发布按钮。
                </template>
            </a-alert>
...
const state = reactive<any>({
    ...
    formDataList: [],
    templatetypeList: [],
    contenttypeList: [{ label: '布局', value: 0 }, { label: '内容', value: 1 }],
    ownertypeList: [],
...
const getUploadZipInfo = async (zipPath: any) => {
    ...
    const res = await resourcePublish.requestZipInfo({ zipPath, zipType: 'template' })
    ...
    if (res.success) {
        ...
        state.formDataList[index].businessId = businessId
        state.formDataList[index].markdownPath = markdownPath
        ...
        state.formDataList[index].type = getTypeCode(resourceInfo?.typeText)
        state.componentList = componentList
        state.noExistsComponentList = noExistsComponentList
        state.formDataList[index].currentZipPath = [{ name: route.query?.fileName, url: zipPath }]
        state.formDataList[index].zipFilename = route.query?.fileName
    } else {
        message.error(res.message)
    }
    state.formDataList[index].formLoading = false
}
const getTypeCode = (type: any) => {
    if (type === '门户组件') {
        return 'portal'
    } else if (type === '表单组件') {
        return 'portal'
...
const getDistinfo = async () => {
    const index = currentIndex.value
    state.formDataList[index].formLoading = true
    const res = await resourcePublish.requestCustomDistInfo({ id: state.formDataList[index].id })
    state.formDataList[index].formLoading = false
    const publishRangeList = res.result.customInfo.publishRange ? res.result.customInfo.publishRange.split(',') : []
    const publishRangeNameList = res.result.customInfo.publishRangeName ? res.result.customInfo.publishRangeName.split(',') : []
    ...
    if (res.success) {
        state.formDataList[index] = {
            ...state.formDataList[index],
            ...res.result,
            ...res.result.customInfo,
            tags: res.result.customInfo.tags ? res.result.customInfo.tags.split(',') : [],
            publishRange,
            coverPath: res.result.customInfo.coverPath ? [{ src: res.result.customInfo.coverPath }] : [],
            cooperationUserIds: res.result.cooperationUserList?.map((item: any) => ({ value: item.id, label: item.nickname })) || [],
            currentZipPath: res.result.customInfo.currentZipPath ? [{ name: res.result.customInfo.currentZipFilename, url: res.result.customInfo.currentZipPath }] : []
        }
        if (route.query?.filePath) {
            state.formDataList[index].currentZipPath = [{ name: route.query?.fileName, url: route.query?.filePath }]
        }
    } else {
        message.error('获取模板页面信息失败！')
    }
}
const handlePublishCancel = async () => {
    const index = currentIndex.value
    const title = '确定要取消发布当前模板页面吗？'
    // const content = '该页面下的所有已创建组件也将同时取消发布！'
    const confirmRes = await showConfirm(title, '', ExclamationCircleOutlined)
    if (confirmRes) {
        state.formDataList[index].formLoading = true
        const res = await resourcePublish.requestChangeStatus({ id: state.formDataList[index].id, status: 0, type: 'template' })
        state.formDataList[index].formLoading = false
        if (res.success) {
...
const onFinish = async (data: any, i?: any) => {
    ...
    const params = {
        ...
        currentZipPath: data.currentZipPath[0]?.url || '',
        currentZipFilename: data.currentZipPath[0]?.name || ''
    }
    state.formDataList[index].formLoading = true
    const res = await resourcePublish.requestSaveCustomComponent(params)
    state.formDataList[index].formLoading = false
    ...
    if (res.success) {
        ...
        judgeShowNoExistsComponentTip(params.currentZipPath)
    } else {
        if (data.status) {
...
const getTemplatetypeList = async () => {
    ...
    if (res.success) {
        state.templatetypeList = res.result.map((item: any) => ({
            ...item,
...
const getTagsList = async () => {
    return await coreApi.requestWebInfoListByGroup({ groupCd: 'templateTags' })
}
