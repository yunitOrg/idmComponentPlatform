    ...
    <div class="cp-upload" style="width:960px">
            ...
            <ARow>
                <ACol :span="7">
                    <div class="title-left">
                        <div class="name">{{ !state.queryId ? '发布业务组件' : '编辑业务组件' }}</div>
                    </div>
                </ACol>
                <ACol :span="17">
                    ...
                    <ASpin :spinning="formData.formLoading">
                        ...
                        >
                            <a-alert v-if="state.noExistsComponentList.length>0" type="warning" show-icon closable :style="{ marginBottom: '20px' }">
                                <template #message>
                                    该业务组件包内包含了{{ state.componentList.length }}个组件，其中{{ state.noExistsComponentList.length }}个组件不存在 <a href="javascript:;" @click="openDetailModal">查看</a>，但不影响业务组件的在线预览（缺少的组件建议还是维护上云）
                                </template>
                            </a-alert>
                            ...
                            <a-form-item name="currentRemark" label="业务组件简介">
                                <a-textarea v-model:value="formData.currentRemark" placeholder="请输入" />
                            </a-form-item>
                            <a-form-item name="tags" label="业务组件标签">
                                <AsyncSelect
                            ...
                            >
                                <IFileUpload
                                    ...
                                    :paramsData="{upFileType: 'bcomponent'}"
                                />
                            </a-form-item>
                            <a-form-item name="markdownContent" extra="文档编辑后会存入表单，随表单一并保存" label="业务组件详情文档">
                            ...
                            <a-form-item :wrapper-col="{ span: 24}" :style="{ padding: '20px 0','text-align': 'center' }">
                                ...
                                <a-button html-type="submit" :style="{ marginLeft: '20px' }" @click="() => formData.status = undefined">保存业务组件</a-button>
        ...
        >
            <a-alert type="warning" show-icon closable :style="{ marginBottom: '20px' }">
                <template #message>
                    该业务组件包内包含了{{ state.componentList.length }}个组件，其中{{ state.noExistsComponentList.length }}个组件不存在，但不影响业务组件的在线预览（缺少的组件建议还是维护上云），如果已经发布了不存在的组件，请再次点击发布按钮。
                </template>
            </a-alert>
...
const getUploadZipInfo = async (zipPath: any) => {
    ...
    const res = await resourcePublish.requestZipInfo({ zipPath, zipType: 'bcomponent' })
...
const getDistinfo = async () => {
    ...
    if (res.success) {
        ...
        message.error('获取业务组件信息失败！')
    }
}
const handlePublishCancel = async () => {
    const index = currentIndex.value
    const title = '确定要取消发布当前业务组件吗？'
    // const content = '该页面下的所有已创建组件也将同时取消发布！'
    const confirmRes = await showConfirm(title, '', ExclamationCircleOutlined)
    if (confirmRes) {
        state.formDataList[index].formLoading = true
        const res = await resourcePublish.requestChangeStatus({ id: state.formDataList[index].id, status: 0, type: 'businesscomponent' })
        state.formDataList[index].formLoading = false
        if (res.success) {
...
const getTagsList = async () => {
    return await coreApi.requestWebInfoListByGroup({ groupCd: 'businesscomponentTags' })
}
