<template>
    <div v-show="pageData.hasPermission && !pageData.isFirst" class="page-max-width package-page flex">
        ...
        <ARow :gutter="[20, 20]" class="flex-1">
            <!-- left -->
            <ACol :span="17">
                <TitleInfoBox type="page" :componentProp="pageData.pageDetail.dataInfo" :showSplit="false" :showVersion="false" showIdText="ID" :showComType="false" :codePackageProp="pageData.pageDetail.dataInfo"></TitleInfoBox>
                ...
                <MdContent title="模板详情文档" :codePackageProp="pageData.pageDetail.dataInfo" :contentUrl="pageData.contentUrl"></MdContent>
                <CommentBox title="模板" commentTypeId="id" articleType="9"></CommentBox>
            </ACol>
            <!-- right -->
            <ACol :span="7">
...
// 取消收藏
const cancelCollect = () => {
    useUserApi.requestUserUnCollect({ sourceId: pageData.pageDetail.dataInfo.id, sourceType: '9' }).then((res) => {
        if (res.success) {
            pageData.pageDetail.dataInfo.collectNumber -= 1
        }
    })
