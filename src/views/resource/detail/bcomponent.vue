<template>
    <div v-show="pageData.hasPermission && !pageData.isFirst" class="page-max-width package-page flex">
        ...
        <ARow :gutter="[20, 20]" class="flex-1">
            <!-- left -->
            <ACol :span="17">
                <TitleInfoBox type="page" :componentProp="pageData.pageDetail.dataInfo" :showcomLangue="false" :showSplit="false" :showVersion="false" showIdText="ID" :showComType="false" :codePackageProp="pageData.pageDetail.dataInfo"></TitleInfoBox>
                ...
                <MdContent title="业务组件详情文档" :codePackageProp="pageData.pageDetail.dataInfo" :contentUrl="pageData.contentUrl"></MdContent>
                <CommentBox title="业务组件" commentTypeId="id" articleType="10"></CommentBox>
            </ACol>
            <!-- right -->
            <ACol :span="7">
    ...
    useHomePageApi
        ...
        .then((res) => {
            ...
            if (res.success) {
                ...
                pageData.contentUrl = `/markdown/custom/${route.query.id}/index.md`
                getComponentList()
            } else {
...
// 点赞
const handlePraise = () => {
    useHomeActionApi[pageData.pageDetail.isPraise ? 'requestCustomUnPraise' : 'requestCustomPraise']({
...
// 取消收藏
const cancelCollect = () => {
    useUserApi.requestUserUnCollect({ sourceId: pageData.pageDetail.dataInfo.id, sourceType: '10' }).then((res) => {
        if (res.success) {
            pageData.pageDetail.dataInfo.collectNumber -= 1
        }
    })
