<template>
    <div class="component-package-manage-list">
        <ManageListHeader title="业务组件管理" btn-text="发布业务组件" :on-btn-click="handleHeaderBtnClick" />
        <div class="content">
...
const handleHeaderBtnClick = () => {
    window.open('/resourcePublish?type=bcomponent')
}
...
const handleCardClick = (data: any) => {
    window.open(`/bcomponentDetail?componentId=${data.id}`)
}
const handleEditClick = (data: any) => {
    window.open(`/bcomponentPublish?id=${data.id}`)
}
const handleCopyClick = (data: any) => {
    const url = `/bcomponentDetail?componentId=${data.id}`
    copyToBoard(url)
...
const handleDeleteClick = async (data: any) => {
    const title = '确定要删除当前业务组件吗？'
    const content = ''
    const answer = await showConfirm(title, content)
    if (answer) {
        state.loading = true
        resourcePublish.requestDeleteChangeCustomComponent({ ids: data.id, datatype: 1 }).then((res) => {
            if (res.success) {
